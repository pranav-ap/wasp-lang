...
struct CFGContext
{
	...
	CFGNode_ptr condition_node;
	CFGNode_ptr end_node;
};

using CFGContext_ptr = std::shared_ptr<CFGContext>;

class CFGBuilder
{
	CFG_ptr module_cfg;
	std::stack<CFGContext_ptr> context_stack;
	...
	void set_current_node(CFGNode_ptr node);
	void set_condition_node(CFGNode_ptr node);
	void set_end_node(CFGNode_ptr node);
...
public:
	CFGBuilder() : next_id(0) {};
	CFG_ptr execute(const Module_ptr module_ast);
};
