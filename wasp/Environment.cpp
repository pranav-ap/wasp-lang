...
AliasInfo* Environment::get_alias_info(std::string name)
{
	auto info = get_info(name);
	ASSERT(holds_alternative<AliasInfo>(*info), name + " is not an Alias!");

	return get_if<AliasInfo>(&*info);
}
...
MapObject* Environment::get_mutable_UDT_variable(string name)
{
	auto info = get_variable_info(name);
...
MapObject* Environment::get_mutable_map_variable(string name)
{
	auto info = get_variable_info(name);
